...
export default {
  ...
  mutations: {
    ...
    incrementQty(state, payload) {
      ...
      incrementData.orders_qty += 1
    },
    decrementQty(state, payload) {
      const decrementData = state.cart.find(
        value => value.product_id === payload.product_id
      )
      decrementData.orders_qty -= 1
    }
  },
  actions: {
    postOrder(context, payload) {
      return new Promise((resolve, reject) => {
        axios
          .post(`${process.env.VUE_APP_URL}/orders`, payload)
          .then(response => {
  ...
  getters: {
    ...
    computedSum(state) {
      const sums = state.cart.map(value => {
        return value.orders_qty * value.product_price
      })
